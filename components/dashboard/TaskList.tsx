    ...
    <div className="space-y-2 p-2">
      {tasks.map((task) => (
        <button
          ...
          className="group w-full overflow-hidden rounded-xl border border-zinc-800/80 bg-zinc-900/70 px-3 py-2.5 text-left transition hover:border-amber-300/35 hover:bg-zinc-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300/50"
        >
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <p
                title={task.title}
                className="max-w-full whitespace-normal break-words text-sm font-medium leading-snug text-zinc-100 transition group-hover:text-amber-100"
              >
                {task.title}
              </p>
              {task.config.scope ? (
