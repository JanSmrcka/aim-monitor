...
describe("Sidebar", () => {
  it("renders New Monitor button", () => {
    render(<Sidebar tasks={[]} isLoading={false} onNewChat={vi.fn()} onSelectTask={vi.fn()} />);
    expect(screen.getByRole("button", { name: /new monitor/i })).toBeInTheDocument();
  });

  it("renders task items", () => {
    render(<Sidebar tasks={mockTasks} isLoading={false} onNewChat={vi.fn()} onSelectTask={vi.fn()} />);
  ...
  it("calls onNewChat when button clicked", async () => {
    ...
    render(<Sidebar tasks={[]} isLoading={false} onNewChat={onNewChat} onSelectTask={vi.fn()} />);
  ...
  it("shows loading skeleton when isLoading", () => {
    render(<Sidebar tasks={[]} isLoading={true} onNewChat={vi.fn()} onSelectTask={vi.fn()} />);
